...
import { Headphones, MessageCircle, ThumbsUp, AlertTriangle, User, Bot, Send, MessageSquare } from 'lucide-react'
import { cn } from '@/lib/utils'
import { useState } from 'react'
import { DifyChatbot } from './dify-chatbot'
...
const conversations = [
  ...
  },
  {
    id: 4,
    type: 'ai',
    message: '换货流程非常简单：1. 登录您的账户 2. 进入"我的订单" 3. 选择需要换货的商品 4. 提交换货申请。我们收到商品后会在3个工作日内为您发送新商品。',
    time: '10:32:29',
    confidence: 0.92,
    knowledgeHit: '换货流程指南'
  },
      ...
      <div className="glass-card rounded-2xl p-6 border border-border/50">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-semibold text-foreground">实时对话监控</h3>
              <p className="text-xs text-muted-foreground">AI客服对话统计</p>
            </div>
          </div>
        ...
        <div className="mt-4 pt-4 border-t border-border/30">
          <div className="grid grid-cols-4 gap-2">
            ...
            <button 
              ...
              className="flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-purple-500/10 border border-purple-500/20 text-purple-500 text-xs font-medium hover:bg-purple-500/20 transition-colors"
            >
              <MessageSquare className="h-3.5 w-3.5" />
              {showDifyChat ? '隐藏' : '打开'} Dify
            </button>
          </div>
        </div>
      </div>

      {/* Dify Chatbot Section */}
      {showDifyChat && (
        <div className="space-y-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-semibold text-foreground">Dify AI 智能助手</h3>
              <p className="text-xs text-muted-foreground">由 Dify 驱动的 AI 客服机器人</p>
            </div>
          </div>
          <DifyChatbot height="700px" />
        </div>
      )}
    </div>
  )
